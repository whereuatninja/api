...
module.exports.findLocationsByUserId = function( userId, after, before, callback ) {
    onConnect(function (err, connection) {
        var query;
        if(after && before){
            query = r.db(dbConfig.db)
                ...
                .filter(function(location){
                    return location('time')
                        .during(r.epochTime(after), r.epochTime(before))
                })
                .orderBy(r.desc('time'))
                .limit(100);
        }
        else{
            query = r.db(dbConfig.db).table('locations').getAll( userId, { index: 'user_id' } ).orderBy(r.desc('time')).limit(100);
        }
        query
